...
export default function EditorActionsDropdown() {
    ...
    const getActiveActionItem=()=>{
      const activeActionItem = dropdownActions.find(item => item.active(editor))
      if(activeActionItem){
            ...
            <MenuList  rounded={'xl'} px={2} gap={1} display={'flex'} flexDir={'column'}>
              {dropdownActions?.map((item, index) => (
                  item.label==='Insert Image'? <CldUploadWidget key={index} uploadPreset={process.env.NEXT_PUBLIC_CLOUDINARY_PRESET||"post_images"} onSuccess={(image: CloudinaryUploadWidgetResults) => {
                    ...
                    {({ open }) => {
                      return (
                         <MenuItem onClick={() => item?.action?.({editor,open})} color={item?.active(editor) ? activeTextColorValue : undefined} bg={item?.active(editor) ? 'blue.500' : undefined} key={index} icon={item.icon && React.createElement(item.icon, { size: 20 })} rounded={'xl'}>
...
:            
                <MenuItem onClick={() => item?.action?.({editor})} color={item?.active(editor) ? activeTextColorValue : undefined} bg={item?.active(editor) ? 'blue.500' : undefined} key={index} icon={item.icon && React.createElement(item.icon, { size: 20 })} rounded={'xl'}>
                  <Text as={'span'} fontSize={'16px'} fontWeight={500}>{item.label}</Text>
                </MenuItem>
              ))}
            </MenuList>
